		...
		<div class="content-wrapper">
			...
			<div class="content-header">
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							<h1 class="m-0">Schedule Messages</h1>
						</div><!-- /.col -->
						<div class="col-sm-6">
							<ol class="breadcrumb float-sm-right">
								<li class="breadcrumb-item"><a href="#">Dashboard</a></li>
								<li class="breadcrumb-item"><a href="#">Inbox</a></li>
								<li class="breadcrumb-item active">Schedule Messages</li>
							</ol>
						</div><!-- /.col -->
					</div><!-- /.row -->
				</div><!-- /.container-fluid -->
						...
						<div class="col-12">
							<div class="card">
								<div class="card-header row align-items-center justify-content-between">
									<div class="col-md-6">
										<h5 class="card-heading d-inline">Schedule Messages</h5>
									</div>
									<div class="col-md-6">
								...
								<div class="card-body p-0 dropdown" style="height: 400px; max-height: 100%; overflow-y: auto; position: relative;">
									...
									<div class="table-responsive mailbox-messages">
										<table class="table table-hover table-sm">
											<tbody id="display_schedule_emails">
											</tbody>
										</table>
									</div>
		...
		$(document).ready(function(){
			...
			$('#refresh_inbox').on('click', function(){
				if(user_info.user_role.inbox.sent == 1) {
					display_schedule_emails($('#main_account_select').val());
				} else {
					$('#display_schedule_emails').html("<tr><td colspan='7' class='text-center'>You're not allowed to read mails</td></tr>");
				}
			});
			...
			if(user_info.user_role.inbox.sent == 1) {
				...
				$('#main_account_select').html("<option value=''>You're not allowed to read accounts</option>");
			}
			...
			$(document).on('click', '.next_btn', function(){
				...
				if(current_page <= account_inbox.total_pages) {
					if(user_info.user_role.inbox.sent == 1) {
						display_schedule_emails($('#main_account_select').val(), current_page);
					} else {
						$('#display_schedule_emails').html("<tr><td colspan='7' class='text-center'>You're not allowed to read mails</td></tr>");
					}
			...
			var account_schedule_function = (account_id, page, per_page_record, q = '') => {
				var inbox;
				if(account_id != '') {
					if(q == '') {
						$.ajax({
							...
							data: JSON.stringify({ 'action' : 'get_schedule', 'mailbox' : 'sent', 'account_id' : account_id, 'user_id' : user_info.id, 'page' : page, 'per_page_record' : per_page_record, 'q':q }),
							success: function(result) {
								inbox = result;
								$('#loader').hide();
							}
			...
			function display_schedule_emails(account_id, page = 1, per_page = per_page_record, q = '') {
				schedule_inbox = account_schedule_function(account_id, page, per_page, q);

				$('#display_schedule_emails').html('');
				if(schedule_inbox.status_code == '001') {
					$('.total_record').html(schedule_inbox.total_record);
					$('.display_record').html((schedule_inbox.offset + 1) +'-'+ (Object.keys(schedule_inbox.display_records).length + schedule_inbox.offset));
					if(page == 1) {
						$('.previous_btn').addClass('disabled');
					} else {
						$('.previous_btn').removeClass('disabled');
					}
					if(page == schedule_inbox.total_pages) {
						$('.next_btn').addClass('disabled');
					} else {
						$('.next_btn').removeClass('disabled');
					}
					$.each(schedule_inbox.display_records, function(key, value) {

						$('#display_schedule_emails').append(
							'<tr style="cursor: pointer;">' + 
								...
								'<td style="min-width: 200px; max-width: 100%"><small class="badge badge-warning">'+ value.schedule_time +'</small></td>' + 
								'<td>' + 
					...
					$('#display_schedule_emails').html("<tr><td colspan='7' class='text-center'>No Record Found</td></tr>");
				}
			}
			if(user_info.user_role.inbox.sent == 1) {
				display_schedule_emails($('#main_account_select').val());
			...
			$('#search_inbox').on('keyup', function(){
				...
				if(q != '') {
					display_schedule_emails($('#main_account_select').val(), page = 1, per_page = per_page_record, q);
				}
			});
