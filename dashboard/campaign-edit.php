...
<body class="hold-transition sidebar-mini sidebar-collapse">
		...
		<div class="content-wrapper">
			...
			<div class="content-header">
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							<h1 class="m-0 d-inline">Campaign Edit</h1>
						...
						<div class="col-12">
							<div class="card">
								<div class="card-header row align-items-center justify-content-between">
									<div class="col-md-6">
										<h5 class="card-heading d-inline">Campaign Edit</h5>
									</div>
									<div class="col-md-6">
									</div>
								</div>
								<div class="card-body dropdown" style="height: 400px; max-height: 100%; overflow-y: auto;">
									...
									<div id="edit_campaign_form_msg"></div>

									<form class="form" id="edit_campaign_form" method="post">
										<input type="hidden" value="<?php echo $_GET['id']; ?>" id="campaign_id" name="">
										...
										<button class="btn btn-dark" id="edit_campaign_form_btn" name="edit_campaign_form_btn" type="submit">Submit</button>
	...
	<script type="text/javascript">
		...
		if(user_info.user_role.campaigns.edit == 0) {
		...
		$(document).ready(function(){
			...
			if(user_info.user_role.campaigns.edit == 1) {
				$.ajax({
					url: api_url,
					type: 'POST',
					data: JSON.stringify({ 'action':'single_campaign', user_id:user_info.id, id:<?php if(isset($_GET['id'])) echo $_GET['id']; ?> }),
					success: function(result) {
						if(result.status_code == '001') {
							$('#campaign_title').val(result.display_records.campaign_title);
							$('#campaign_description').val(result.display_records.campaign_description);
							display_contact_lists(result.display_records.contact_list_id);
							display_main_account_select(result.display_records.account_id);
							$('#subject').val(result.display_records.subject);
							if(result.display_records.is_schedule == 1) {
								...
								console.log(result.display_records.schedule_time);

								$("#schedule_time").flatpickr({
									defaultDate: result.display_records.schedule_time, 
									enableTime: true,
									dateFormat: "d-m-Y H:i",
									minDate: result.display_records.schedule_time,
									time_24hr: true
								});
							}
							$('#content').html(result.display_records.content);
							$('#content_type').val(result.display_records.content);
							$('#attachments').val(result.display_records.campaign_attachment_details);
						}
					}
				});
			} else {
		...
		function display_contact_lists(contact_list_id) {
			...
			if(contact_lists.status_code == '001') {
				...
				$.each(contact_lists.display_records, function(key, value) {
					...
					if(contact_list_id == value.id) { var contact_list_selected = 'selected'; } else { var contact_list_selected = ''; }

					$('#contact_list_id').append(
						'<option '+ contact_list_selected +' value="'+ value.id +'">'+ value.contact_list_title +' ('+ value.total_contacts +')</option>'
					);
				});
			} else {
		...
		function display_main_account_select(account_id) {
			$('#account_id').html('');
		...
		const dropzone = new Dropzone("div.dropzone", {
			...
			removedfile: function(file) {
				...
				$.ajax({
					...
					success: function(data) {
						var attached_files = $('#attachments').val();
			...
			success: function(file, response) {
				let success_attached_files = $('#attachments').val();
				if(success_attached_files != '') {
					success_attached_files = success_attached_files + ', ' + response;
				} else {
					success_attached_files = response;
				}
				$('#attachments').val(success_attached_files);
			}
		});
			...
			if(bool == 0) {
				if(user_info.user_role.campaigns.edit == 1) {
					$.ajax({
						...
						type: 'POST', 
						beforeSend: function() {
							$('#edit_campaign_form_msg').removeClass('alert-danger').addClass('alert alert-success').text('Please Wait...');
							$('#edit_campaign_form_btn').addClass('disabled');
						},
						data: JSON.stringify({ action:'edit_campaign', campaign_id:campaign_id, user_id:user_info.id, campaign_title:campaign_title, campaign_description:campaign_description, contact_list_id:contact_list_id, account_id:account_id, subject:subject, content:content, href_links:href_links, attachments:attachments, is_schedule:is_schedule, schedule_time:schedule_time }), 
						success: function(result) {
							if(result.status_code == '001') {
								$('#edit_campaign_form_msg').removeClass('alert-danger').addClass('alert alert-success').text('Campaign Successfully Created');
								setTimeout(function(){
									window.location.href = dashboard_url+'campaigns.php';
								}, 1000);
							} else if(result.status_code == '002') {
								$('#edit_campaign_form_msg').removeClass('alert-success').addClass('alert alert-danger').text('Please Try Again');
							} else if(result.status_code == '003') {
								$('#edit_campaign_form_msg').removeClass('alert-success').addClass('alert alert-danger').text('Please Fill Required Fields');
							}
							$('#edit_campaign_form_btn').removeClass('disabled');
						}
					});
				} else {
					alert("You're not allowed to edit campaign");
					window.top.location = dashboard_url +'campaigns.php';
				}
			}
		});
