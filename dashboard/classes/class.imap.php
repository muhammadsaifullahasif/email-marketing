 ...
 * Usage example:
  ...
  3. $messages = $imap->searchMessages('text'); // Array of messages
...
class Imap
{
    ...
    {
        if (!file_exists('attachments/'.$username)) {
            mkdir('attachments/'.$username, 0777, true);
            $this->attachments_dir = 'attachments/'.$username;
        } else {
            $this->attachments_dir = 'attachments/'.$username;
        }
    }
    ...
    public function getMessages($type = 'html', $sort = 'asc', $nStart = 0, $nCnt = 0)
    {
        $stream = $this->imapStream;
        if($nCnt == 0) {
            $nCnt = imap_num_msg($stream);
        }
        // $this->emails = imap_search($stream, 'ALL');
        $this->emails = imap_fetch_overview($stream, ($nStart+1).':'.($nStart+$nCnt));
        ...
        if ($this->emails) {
            ...
            foreach ($this->emails as $email_number) {
                ...
                // $messages[$i] = $email_number->uid;
                $uid = $email_number->uid;
                // $uid = imap_uid($stream, $email_number);
                // $messages[$i] = $email_number;
                $messages[$i] = $this->loadMessage($uid, $type, $email_number);
                if ($i == $this->limit) {
                    break;
                }
                $i++;
    ...
    public function searchMessages($type = 'text', $sort = 'asc')
    {
        $stream = $this->imapStream;
        $this->emails = imap_search($stream, 'ALL');
        if (strtolower($sort) == 'desc') {
     ...
     * returned array from - searchMessages
