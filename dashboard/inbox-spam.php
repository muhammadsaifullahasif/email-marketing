						...
						<div class="col-12">
							<div class="card">
								<div class="card-header row align-items-center justify-content-between">
									<div class="col-md-6">
										<h5 class="card-heading d-inline">Spam</h5>
									</div>
									<div class="col-md-6">
		...
		$(document).ready(function(){
			...
			$('#refresh_spam').on('click', function(){
				if(user_info.user_role.inbox.read == 1) {
					display_spam_emails($('#main_account_select').val());
				} else {
					$('#display_spam_emails').html("<tr><td colspan='7' class='text-center'>You're not allowed to read mails</td></tr>");
				}
			});
			...
			function refresh_spam() {
				...
				$.ajax({
					...
					success: function(result) {
						if(user_info.user_role.inbox.read == 1) {
							display_spam_emails(account_id);
						} else {
							$('#display_spam_emails').html("<tr><td colspan='7' class='text-center'>You're not allowed to read mails</td></tr>");
						}
					}
			...
			$(document).on('click', '.refresh_spam', refresh_spam);
			...
			$(document).on('click', '.next_btn', function(){
				...
				if(current_page <= account_inbox.total_pages) {
					if(user_info.user_role.inbox.read == 1) {
						display_spam_emails($('#main_account_select').val(), current_page);
					} else {
						$('#display_spam_emails').html("<tr><td colspan='7' class='text-center'>You're not allowed to read mails</td></tr>");
					}
			...
			var account_spam_function = (account_id, page, per_page_record, q = '') => {
				var inbox;
				if(account_id != '') {
					if(q == '') {
						$.ajax({
							...
							data: JSON.stringify({ 'action' : 'get_inbox', 'mailbox' : 'spam', 'account_id' : account_id, 'user_id' : user_info.id, 'page' : page, 'per_page_record' : per_page_record }),
							success: function(result) {
								inbox = result;
								$('#loader').hide();
							}
						...
						$.ajax({
							...
							data: JSON.stringify({ 'action' : 'get_inbox', 'mailbox' : 'spam', 'account_id' : account_id, 'user_id' : user_info.id, 'page' : page, 'per_page_record' : per_page_record, 'q':q }),
							success: function(result) {
								inbox = result;
								$('#loader').hide();
							}
			...
			function display_spam_emails(account_id, page = 1, per_page = per_page_record, q = '') {
				spam_inbox = account_spam_function(account_id, page, per_page, q);

				$('#display_spam_emails').html('');
				if(spam_inbox.status_code == '001') {
					$('.total_record').html(spam_inbox.total_record);
					$('.display_record').html((spam_inbox.offset + 1) +'-'+ (Object.keys(spam_inbox.display_records).length + spam_inbox.offset));
					if(page == 1) {
						$('.previous_btn').addClass('disabled');
					} else {
						$('.previous_btn').removeClass('disabled');
					}
					if(page == spam_inbox.total_pages) {
						$('.next_btn').addClass('disabled');
					} else {
						$('.next_btn').removeClass('disabled');
					}
					$.each(spam_inbox.display_records, function(key, value) {

						$('#display_spam_emails').append(
					...
					$('#display_spam_emails').html("<tr><td colspan='7' class='text-center'>No Record Found</td></tr>");
				}
			}
			if(user_info.user_role.inbox.read == 1) {
				display_spam_emails($('#main_account_select').val());
			...
			$('#search_inbox').on('keyup', function(){
				...
				if(q != '') {
					display_spam_emails($('#main_account_select').val(), page = 1, per_page = per_page_record, q);
				}
			});
